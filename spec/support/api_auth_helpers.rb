module ApiAuthHelpers
  ...
  def api_log_in_as(user_or_email, password: 'secret123')
  ...
  def api_token_for(user, password: 'secret123')
    api_log_in_as(user, password: password)
    api_response_json[:user][:token]
  end
end
