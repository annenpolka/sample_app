...
RSpec.describe 'API JWT Authentication', type: :request do
  ...
  let(:token_for_admin) { api_token_for(admin) }
  let(:token_for_non_admin) { api_token_for(non_admin) }

  describe 'POST /api/v1/auth/login' do
    context 'with valid credentials' do
      it 'returns JWT token' do
        api_log_in_as(user, password: 'secret123')
        expect(response).to have_http_status(:ok)
        expect(api_response_json.dig(:user, :token)).to be_present
      end
    end

    context 'with invalid credentials' do
      it 'returns 401' do
        api_log_in_as(user, password: 'invalid')
        expect(response).to have_http_status(:unauthorized)
        expect(api_response_json.dig(:user, :token)).not_to be_present
      end
    end
  end
  ...
  describe 'GET /api/v1/users' do
    context 'when authenticated' do
      it '一覧を返す（ページネーション情報を含む）' do
        ...
        expect(api_response_json[:meta]).to be_present
        expect(api_response_json[:meta][:total_count]).to be_present
      end
    end
  ...
  describe 'GET /api/v1/users/:id' do
    context '公開プロフィール' do
      it 'id指定でユーザー情報を取得する' do
        ...
        expect(api_response_json[:password_digest]).not_to be_present
      end
    end
  end
  ...
  describe 'DELETE /api/v1/users/:id' do
    context 'as admin' do
      ...
      it '自分自身は削除できない' do
        ...
        # まだログイン可能
        api_log_in_as(admin, password: 'secret123')
        expect(response).to have_http_status(:ok)
      end
    end
  ...
  describe 'PATCH /api/v1/users/:id' do
    context 'as self' do
      it '自分の情報を更新できる' do
        ...
        expect(response).to have_http_status(:ok)
        expect(api_response_json.dig(:user, :name)).to eq("新しい名前")
      end
    end
    ...
    context 'as admin' do
      ...
      it '管理者でも admin 属性は更新できない' do
        ...
        expect(response).to have_http_status(:ok)
        expect(api_response_json.dig(:user, :name)).to eq("管理者が更新")
        # Strong Parametersにより admin は無視される
        expect(target.reload.admin).to be false
      end
    end
    ...
    context 'when unauthenticated' do
      it '401 を返し、変更されない' do
        ...
        expect(response).to have_http_status(:unauthorized)
        get api_v1_user_path(user.id), headers: api_headers
        expect(api_response_json[:name]).not_to eq("新しい名前")
      end
    end
  end
end
